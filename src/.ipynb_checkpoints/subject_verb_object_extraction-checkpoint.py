...
import src.help_NER_UB as help_ner
import numpy as np
import spacy
import src.scispacy
...
def printDeps(toks):
    result = []
    for tok in toks:
        result.append([tok.orth_, tok.dep_, tok.pos_, tok.head.orth_, [t.orth_ for t in tok.lefts], [t.orth_ for t in tok.rights]])
    return result
